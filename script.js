(() => {
  // Marca que a intro já foi visualizada na sessão atual.
  // Assim, se o usuário voltar para o index a partir de outra página, a intro não carrega de novo.
  ...
  // ==============================================
  // Lógica para a Página Única de Produtos
  // ==============================================
  const kitViews = document.querySelectorAll('.kit-view');
  
  if (kitViews.length > 0) {
    function updateProductView() {
      // Pega o hash da URL (ex: #lite, #plus, #pro)
      let hash = window.location.hash || '#lite';
      
      // Se for um hash desconhecido, volta para o lite
      if (!['#lite', '#plus', '#pro'].includes(hash)) {
        hash = '#lite';
      }
      
      // Esconde todos e mostra apenas o correto
      kitViews.forEach(view => {
        view.classList.remove('active');
        if (view.id === 'view-' + hash.substring(1)) {
          view.classList.add('active');
        }
      });
      
      // Para garantir que a página inicia no topo ao trocar a aba do produto
      window.scrollTo(0, 0);
    }

    // Escuta por mudanças na hash (quando clica num botão "Ver produto" de outra parte ou aba)
    window.addEventListener('hashchange', updateProductView);
    
    // Roda logo que a página carrega
    updateProductView();
  }

  // Lógica de Galeria de Imagens de Produtos (troca a imagem principal ao clicar na miniatura)
  document.querySelectorAll('.product-gallery').forEach(gallery => {
    const mainImg = gallery.querySelector('.main-image-slot img');
    const mainSlot = gallery.querySelector('.main-image-slot');
    const thumbs = gallery.querySelectorAll('.thumb-slot');
